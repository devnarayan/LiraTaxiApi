...
$(document).scroll(function() {
    $('#menu').toggle($(this).scrollTop()>300)
 })
...
$('.ert').click(function () {
    zin = zin+1;
    $('.header').find('#video_background').remove();
    ...
    if($(this).hasClass('aa')){
        ...
        $('.pr_btns').slideUp('slow');
        $('.dr_btns').slideDown('slow');
    }
    else{
        if (matchMedia('(min-width: 640px)').matches) {
        ...
        $('.dr_btns').slideUp('slow');
        $('.pr_btns').slideDown('slow');
    }
    $("#video_background").css("z-index", zin);
    $(".bgg").removeClass('bgg');
